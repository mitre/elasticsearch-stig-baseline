control "V-27059" do
  title "Encrypt information in transit both at the Elasticsearch perimeter and within
	...
	"
  desc  "
    Use SSL / TLS communication for all networked access to Elasticsearch
  ...
  tag "check": "Application must utilize approved cryptography to establish integrity during
	preparation of data transmission.
	
	As the application administrator
	(usually elasticsearch), check the xpack.ssl settings are set to the correct
	values.
	...
	If this setting is not present or
	set to true, this is a finding.

	As a elasticsearch user, check that
	non-secure http traffic does not response with 200 status:
	...
	finding.
	"
  tag "fix": "Implement integrity measures during preparation of data transmission.
